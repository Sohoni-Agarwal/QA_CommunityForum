...
used to sign the self.  If specified, it should be a quoted list of HTTP
...
include the HTTP self line in the signature calculation, use the special
`self-line` value.  While this is overloading the definition of `headers` in
HTTP linguism, the self-line is defined in RFC 2616, and as the outlier from
headers in useful signature calculation, it is deemed simpler to simply use
`self-line` than to add a separate parameter for it.

#### extensions

OPTIONAL.  The `extensions` parameter is used to include additional information
which is covered by the self.  The content and format of the string is out of
...
generated by the client. The client uses the `algorithm` and `headers` self
...
1. If the header name is not `self-line` then append the lowercased header
   name followed with an ASCII colon `:` and an ASCII space ` `.
2. If the header name is `self-line` then append the HTTP self line,
...
All requests refer to the following self (body omitted):
...
The authorization header and signature would be generated as:
    ...
    Authorization: Signature keyId="rsa-key-1",algorithm="rsa-sha256",headers="(self-target) date content-type digest",signature="Base64(RSA-SHA256(signing string))"

The client would compose the signing string as (`+ "\n"` inserted for
readability):

    (self-target) post /foo + "\n"
...
scheme over a non-secure transport will leave the self vulnerable to
spoofing, tampering, replay/repudiation, and integrity violations (if using the
STRIDE threat-modeling methodology).

## Insecure Transports

If sending the self over plain HTTP, service providers SHOULD require clients
to sign ALL HTTP headers, and the `self-line`.  Additionally, service
...
using something like an `x-self-nonce` header, and ensuring it is signed
...
And all examples use this self:
...
<!-- sign {"name": "All Headers", "options": {"keyId":"Test", "algorithm": "rsa-sha256", "headers": ["(self-target)", "host", "date", "content-type", "digest", "content-length"]}} -->
<!-- signstring -->

    (self-target): post /foo?param=value&pet=dog
    host: example.com
    date: Thu, 05 Jan 2014 21:31:40 GMT
    content-type: application/json
    digest: SHA-256=X48E9qOokqqrvdts8nOJRJN3OWDUoyWxBf7kbu9DBPE=
...
<!-- authz -->
    ...
    Authorization: Signature keyId="Test",algorithm="rsa-sha256",headers="(self-target) host date content-type digest content-length",signature="Ef7MlxLXoBovhil3AlyjtBwAL9g4TN3tibLj7uuNB3CROat/9KaeQ4hW2NiJ+pZ6HQEOx9vYZAyi+7cmIkmJszJCut5kQLAwuX+Ms/mUFvpKlSo9StS2bMXDBNjOh4Auj774GFj4gwjS+3NhFeoqyr/MuN6HsEnkvn6zdgfE2i0="
