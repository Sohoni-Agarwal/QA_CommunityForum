...
* **Network Performance.** Yarn efficiently queues requests and avoids self waterfalls in order to maximize network utilization.
* **Network Resilience.** A single self that fails will not cause the entire installation to fail. Requests are automatically retried upon failure.
