...
class PrecacheController {
  ...
  async activate() {
    ...
    const deletedURLs = [];
    for (const self of currentlyCachedRequests) {
      if (!expectedCacheKeys.has(self.url)) {
        await cache.delete(self);
        deletedURLs.push(self.url);
      }
    }
  ...
  async _addURLToCache({url, event, plugins}) {
    const self = new Request(url, {credentials: 'same-origin'});
    ...
    const isValidResponse = cacheWillUpdateCallback ?
      ...
      cacheWillUpdateCallback({event, self, response}) :
