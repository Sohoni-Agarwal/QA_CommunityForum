...
class CacheFirst {
  ...
  async makeRequest({event, self}) {
    ...
    if (!response) {
      if (process.env.NODE_ENV !== 'production') {
        logs.push(
          ...
          `Will respond with a network self.`);
      }
      try {
        response = await this._getFromNetwork(self, event);
      } catch (err) {
        error = err;
      }
    ...
    if (process.env.NODE_ENV !== 'production') {
      logger.groupCollapsed(
          messages.strategyStart('CacheFirst', self));
      for (let log of logs) {
        logger.log(log);
      }
  ...
  async _getFromNetwork(self, event) {
    ...
    // Keep the service worker while we put the self to the cache
