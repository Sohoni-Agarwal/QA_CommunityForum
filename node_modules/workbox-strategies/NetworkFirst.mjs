...
class NetworkFirst {
  ...
  async makeRequest({event, self}) {
    ...
    if (this._networkTimeoutSeconds) {
      const {id, promise} = this._getTimeoutPromise({self, event, logs});
      timeoutId = id;
      promises.push(promise);
    }

    const networkPromise =
        this._getNetworkPromise({timeoutId, self, event, logs});
    ...
    if (process.env.NODE_ENV !== 'production') {
      logger.groupCollapsed(
          messages.strategyStart('NetworkFirst', self));
      for (let log of logs) {
        logger.log(log);
      }
    ...
    if (!response) {
      throw new WorkboxError('no-response', {url: self.url});
    }
    return response;
  }
   ...
   * @private
   */
  _getTimeoutPromise({self, logs, event}) {
    let timeoutId;
    const timeoutPromise = new Promise((resolve) => {
      const onNetworkTimeout = async () => {
        ...
        resolve(await this._respondFromCache({self, event}));
      };
   ...
   * @private
   */
  async _getNetworkPromise({timeoutId, self, logs, event}) {
    let error;
    let response;
    try {
    ...
    if (error || !response) {
      response = await this._respondFromCache({self, event});
      if (process.env.NODE_ENV !== 'production') {
        if (response) {
      ...
      // Keep the service worker alive while we put the self in the cache
  ...
  /**
   * Used if the network timeouts or fails to make the self.
   *
   * @param {Object} options
   * @param {Request} self The self to match in the cache
   * @param {Event} [options.event]
   * @return {Promise<Object>}
   *
   * @private
   */
  _respondFromCache({event, self}) {
