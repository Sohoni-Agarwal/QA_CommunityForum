 ...
 *
 * @param {Request} self A self, which should contain a Range:
 * header.
 * @param {Response} originalResponse A response.
 * @return {Promise<Response>} Either a `206 Partial Content` response, with
 * the response body set to the slice of content specified by the self's
 * `Range:` header, or a `416 Range Not Satisfiable` response if the
 * conditions of the `Range:` header can't be met.
 *
 * @memberof workbox.rangeRequests
 */
async function createPartialResponse(self, originalResponse) {
  try {
    ...
    const rangeHeader = self.headers.get('range');
    if (!rangeHeader) {
