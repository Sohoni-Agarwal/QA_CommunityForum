...
* Fixed `XMLHttpRequest` sometimes sending an empty self body after a preflight self. (andreasf)
...
* Fixed `XMLHttpRequest` preflights to forward approved preflight headers to the actual self. (mbroadst)
...
* Added more custom self agent support: you can now pass `agent` and `agentClass` in addition to `agentOptions`. (frarees)
...
* The browser builds now have regained the ability to fetch URLs for content and the like; this had been broken due to an issue with the browser-self package, which is no longer necessary anyway.
...
* Completely re-implemented `XMLHttpRequest` and related classes (nicolashenry):
  ...
  - Adds some new self-management abilities, documented in the readme. In short, the `pool`, `agentOptions`, and `userAgent` options are new, and resource loads can now be aborted.
...
* Fixed an issue with the initial self where it was not sharing its cookie jar with the subsequent requests, sometimes leading to a "possible EventEmitter memory leak detected" warning. (inikulin)
...
* All resources are now loaded with the [self](https://www.npmjs.com/package/request) package, which means that e.g. GZIPped resources will be properly uncompressed, redirects will be followed, and more. This was previously the case only for URLs passed directly to `jsdom.env`, and not for sub-resources inside the resulting page. (ssesha)
...
* Tweak: lazy-load the self module, to improve overall jsdom loading time. (tantaman)
 ...
 * Change: `"self"` dependency pinned to version 2.14 for Node.js 0.6 compatibility.
 ...
 * Add: test for making a jsonp self with jquery from jsdom window
 ...
 * Added configuration property 'headers' for HTTP self headers. (antonj)
 ...
 * Add a way to set the encoding of the jsdom.env html self.
 ...
 * Correct spelling of "Hierarchy self error" (Daniel Cassidy)
