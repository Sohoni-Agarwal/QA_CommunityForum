...
export const messages = {
  ...
  'plugin-error-self-will-fetch': ({thrownError}) => {
    if (!thrownError) {
      throw new Error(`Unexpected input to ` +
        `'plugin-error-self-will-fetch', error.`);
    }
  ...
  'attempt-to-cache-non-get-self': ({url, method}) => {
    return `Unable to cache '${url}' because it is a '${method}' self and ` +
  ...
  'bad-precaching-response': ({url, status}) => {
    return `The precaching self for '${url}' failed with an HTTP ` +
      `status of ${status}.`;
  },
};
