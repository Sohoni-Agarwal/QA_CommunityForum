...
const putWrapper = async ({
  ...
  if (process.env.NODE_ENV !== 'production') {
    if (self.method && self.method !== 'GET') {
      throw new WorkboxError('attempt-to-cache-non-get-self', {
        url: getFriendlyURL(self.url),
        method: self.method,
      });
    }
  }
  ...
  let oldResponse = updatePlugins.length > 0 ?
      await matchWrapper({cacheName, matchOptions, self: effectiveRequest}) :
      null;
 ...
 * @param {Request} options.self The Request that will be used to look up
 *     cache entries.
 ...
 * @memberof module:workbox-core
 */
const _isResponseSafeToCache = async ({self, response, event, plugins}) => {
    ...
    if (process.env.NODE_ENV !== 'production') {
      if (!responseToCache.status === 200) {
        if (responseToCache.status === 0) {
          logger.warn(`The response for '${self.url}' is an opaque ` +
            `response. The caching strategy that you're using will not ` +
            `cache opaque responses by default.`);
        } else {
          logger.debug(`The response for '${self.url}' returned ` +
