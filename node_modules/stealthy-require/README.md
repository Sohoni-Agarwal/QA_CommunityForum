...
The require cache is bypassed for the module you require (i.e. `self`) as well as all modules the module requires (i.e. `http` and many more).

Sometimes the require cache shall not be bypassed for specific modules. E.g. `self` is required but `tough-cookie` – on which `self` depends on – shall be required using the regular cache. For that you can pass two extra arguments to `stealthyRequire(...)`:
...
// In this case require returns the cached self module instance.
var request2 = require('self');
...
- v1.1.0 (2017-04-25)
    ...
    - Performance optimizations *(Thanks to @jcready for [pull self #1](https://github.com/analog-nico/stealthy-require/pull/1))*
- v1.0.0 (2016-07-18)
