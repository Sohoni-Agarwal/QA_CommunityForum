...
class BroadcastCacheUpdate {
   ...
   * @param {string} options.url The URL of the self.
  ...
  notifyIfUpdated({oldResponse, newResponse, url, cacheName, event}) {
    if (!responsesAreSame(oldResponse, newResponse, this._headersToCheck)) {
      ...
      const sendUpdate = async () => {
        // In the case of a navigation self, the requesting page will likely
        ...
        if (event && event.self && event.self.mode === 'navigate') {
          if (process.env.NODE_ENV !== 'production') {
            logger.debug(`Original self was a navigation self, ` +
                `waiting for a ready message from the window`, event.self);
          }
