...
(function (global, factory) {
  ...
  function deepField(data, propertyPath, propertySearch, propertySearchDepth) {
    var ret = null,
        ...
        itemValue,
        parameter,
        newPropertySearchDepth = -1; // Check if the max-search depth got reached when propertySearch is activated
  ...
  function _getOptions(search, _defaults) {
    var options = {};
    search = search || {}; // did we have a negator?
    //options.negator = search._not ? true : _defaults.negator || false;

    options.negator = _getSingleOpt(search._not, _defaults.negator, false); // do we join via AND or OR
    //options.joinAnd = search._join && search._join === "OR" ? false : _defaults.join || true;

    options.joinAnd = _getSingleOpt(search._join, _defaults.join, "AND") !== "OR"; // did we have text, word, start or end search?
  ...
  var _defaults$1 = {}; // Allows to overwrite the global default values
  ...
  function singleMatch(field, s, text, word, start, end) {
    var oneMatch = false,
        ...
        j,
        from,
        to; // for numbers, exact match; for strings, ignore-case match; for anything else, no match
  ...
  function matchArray(ary, search) {
    var matched = false,
        i,
        ret = [],
        options = _getOptions(search, _defaults$1);
  ...
  function _matchObj(obj, search, options) {
    ...
    search = search || {}; // if joinAnd, then matched=true until we have a single non-match; if !joinAnd, then matched=false until we have a single match

    matched = !!options.joinAnd; // are we a primitive or a composite?
