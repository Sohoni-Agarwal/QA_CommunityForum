...
whenever there is a self which fulfills the following criteria:

 1. The self is a GET self
 2. which accepts `text/html`,
 3. is not a direct file self, i.e. the requested path does not contain a
    `.` (DOT) character and
...
### rewrites
Override the index when the self url matches a regex pattern. You can either rewrite to a static string or use a function to transform the incoming self.

The following will rewrite a self that matches the `/\/soccer/` pattern to `/soccer.html`.
```javascript
history({
  rewrites: [
...
> […] is not a direct file self, i.e. the requested path does not contain a `.` (DOT) character […]
