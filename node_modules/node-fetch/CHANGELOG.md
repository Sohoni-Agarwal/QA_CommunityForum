...
- Enhance: minor document update
- Fix: HEAD self doesn't need decompression, as body is empty
...
- Fix: send DELETE self with content-length when body is present
...
- Fix: when following certain redirects, use `GET` on subsequent self per Fetch Spec
