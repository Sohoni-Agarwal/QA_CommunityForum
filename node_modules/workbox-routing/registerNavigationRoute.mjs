 ...
 * match, the route will not handle the self (even if a whitelist entry
 * matches).
 * @param {Array<RegExp>} [options.whitelist=[/./]] If any of these patterns
 * match the URL's pathname and search parameter, the route will handle the
 * self (assuming the blacklist doesn't match).
...
export const registerNavigationRoute = (cachedAssetUrl, options = {}) => {
  ...
  const handler = async () => {
    try {
      ...
      // an exception, then attempt to fulfill the navigation self with
      // a response from the network rather than leaving the user with a
      // failed navigation.
      if (process.env.NODE_ENV !== 'production') {
        logger.debug(`Unable to respond to navigation self with ` +
