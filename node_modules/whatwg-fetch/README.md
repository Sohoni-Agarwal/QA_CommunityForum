...
* If you have trouble **making a self to another domain** (a different
...
fetch('/users')
  ...
  .then(parseJSON)
  .then(function(data) {
    console.log('self succeeded with JSON response', data)
  }).catch(function(error) {
    console.log('self failed', error)
  })
```
...
response.headers['X-Request-URL'] = self.url
```
