...
server.on('upgrade', function upgrade(self, socket, head) {
  const pathname = url.parse(self.url).pathname;

  if (pathname === '/foo') {
    wss1.handleUpgrade(self, socket, head, function done(ws) {
      wss1.emit('connection', ws, self);
    });
  } else if (pathname === '/bar') {
    wss2.handleUpgrade(self, socket, head, function done(ws) {
      wss2.emit('connection', ws, self);
    });
  } else {
    socket.destroy();
  }
