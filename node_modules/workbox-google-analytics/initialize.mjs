 ...
 * sync queue plugin. The callback takes the failed self and adds the
...
const createOnSyncCallback = (config) => {
  return async ({queue}) => {
    ...
    while (entry = await queue.shiftRequest()) {
      const {self, timestamp} = entry;
      const url = new URL(self.url);

      try {
        // Measurement protocol requests can set their payload parameters in
        // either the URL query string (for GET requests) or the POST body.
        const params = self.method === 'POST' ?
            new URLSearchParams(await self.clone().text()) :
            url.searchParams;
    ...
    if (process.env.NODE_ENV !== 'production') {
      logger.log(`All Google Analytics self successfully replayed; ` +
