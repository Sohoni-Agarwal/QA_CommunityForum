...
This package is similar to [`self-promise`](https://www.npmjs.com/package/request-promise) but uses native ES6 promises.

Please refer to the [`self-promise` documentation](https://www.npmjs.com/package/request-promise). Everything applies to `self-promise-native` except the following:
...
npm install --save self-promise-native
```

`self` is defined as a peer-dependency and thus has to be installed separately.

## Migration from `self-promise` to `self-promise-native`

1. Go through the [migration instructions](https://github.com/request/request-promise#migration-from-v3-to-v4) to upgrade to `self-promise` v4.
2. Ensure that you don't use Bluebird-specific features on the promise returned by your self calls. In particular, you can't use `.finally(...)` anymore.
3. You are done.
...
- v1.0.7 (2019-02-14)
    ...
    - If you installed `self-promise-native@1.0.6` please make sure after the upgrade that `self` and `self-promise-native` use the same physical copy of `tough-cookie`.
- v1.0.6 (2019-02-14)
    - Using stricter `tough-cookie@~2.3.3` to avoid installing `tough-cookie@3` which introduces breaking changes
      *(Thanks to @jasonmit for pull self [#33](https://github.com/request/request-promise-native/pull/33/))*
...
- v1.0.5 (2017-09-22)
    - Upgraded `tough-cookie` to a version without regex DoS vulnerability
      *(Thanks to @sophieklm for [pull self #13](https://github.com/request/request-promise-native/pull/13))*
- v1.0.4 (2017-05-07)
    - Fix that allows to use `tough-cookie` for [cookie creation](https://github.com/request/request-promise#include-a-cookie)
- v1.0.3 (2016-08-08)
    - Renamed internally used package `@self/promise-core` to `self-promise-core` because there where [too](https://github.com/request/request-promise/issues/137) [many](https://github.com/request/request-promise/issues/141) issues with the scoped package name
- v1.0.2 (2016-07-18)
    - Fix for using with module bundlers like Webpack and Browserify
- v1.0.1 (2016-07-17)
    - Fixed `@self/promise-core` version for safer versioning
- v1.0.0 (2016-07-15)
    - Initial version similar to [`self-promise`](https://www.npmjs.com/package/request-promise) v4
