...
the Node.js HTTP self and the `path` is a urlencoded path to send (urlencoded,
...
to "deny").
  ...
  - `'deny'` Send a 403 for any self for a dotfile.
...
given directory as the top-level. For example, a self
