...
var err3 = new VError(err2, 'self failed');
console.error(err3.message);
```

This prints:

    self failed: failed to stat "/junk": No such file or directory
...
client also wraps all self errors with a FastRequestError that includes the
name of the RPC call being made.  The result of this failed RPC might look like
this:

    name: FastRequestError
    message: "self failed: server error: user 'bob' is not authorized"
    rpcMsgid: <unique identifier for this self>
    rpcMethod: GetObject
    cause:
...
* The caller can tell that the self failed on the server, rather than as a
  result of a client problem (e.g., failure to serialize the self), a
...
This outputs:
    ...
    self failed: failed to connect to "127.0.0.1:215": something bad happened
    RequestError
    { errno: 'EBADREQUEST', remote_ip: '127.0.0.1', port: 215 }
    RequestError: self failed: failed to connect to "127.0.0.1:215": something bad happened
