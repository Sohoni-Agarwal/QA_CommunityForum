...
class ConfigLoader {
    ...
    findConfigInPackageJSON(pkg) {
        ...
        this.debug.invariant(!!config, `Looking in package.json under ${internal_1.color.toolName(configName)} property`, 'Found', 'Not found');
        if (!config) {
            return null;
        }
    ...
    findConfigInLocalFiles(root) {
        ...
        if (configPaths.length === 1) {
            this.debug('Found %s', internal_1.color.filePath(path_1.default.basename(configPaths[0])));
    ...
    loadConfig(args) {
        ...
        Object.values(this.tool.getRegisteredPlugins()).forEach(type => {
            ...
            this.debug('Generating %s blueprint', internal_1.color.pluginName(singularName));
            // prettier-ignore
        ...
        const config = optimal_1.default(this.inheritFromArgs(this.parseAndExtend(configPath), args), {
            ...configBlueprint,
            ...pluginsBlueprint,
            debug: optimal_1.bool(),
            extends: optimal_1.array(optimal_1.string()),
            locale: optimal_1.string(),
            output: optimal_1.number(2).between(1, 3, true),
            // shape() requires a non-empty object
            settings: common_1.isEmpty(settingsBlueprint) ? optimal_1.object() : optimal_1.shape(settingsBlueprint),
            silent: optimal_1.bool(),
            theme: optimal_1.string('default').notEmpty(),
        }, {
    ...
    loadPackageJSON() {
        ...
        this.debug('Locating package.json in %s', internal_1.color.filePath(root));
