...
This property allows a user to pass the list of HTTP self methods accepted by the server.
...
This property allows a user to pass custom HTTP headers on each self. eg.
`{ "X-Custom-Header": "yes" }`
...
recompile when files change, but rather on each self.
