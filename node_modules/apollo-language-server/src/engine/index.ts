...
export class ApolloEngineClient extends GraphQLDataSource {
  ...
  willSendRequest(self: any) {
    if (!self.headers) self.headers = {};
    self.headers["x-api-key"] = this.engineKey;
    if (this.clientIdentity && this.clientIdentity.name) {
      self.headers["apollo-client-name"] = this.clientIdentity.name;
      self.headers[
        "apollo-client-reference-id"
      ] = this.clientIdentity.referenceID;
      self.headers["apollo-client-version"] = this.clientIdentity.version;
      return;
    }

    // default values
    self.headers["apollo-client-name"] = "Apollo Language Server";
    self.headers["apollo-client-reference-id"] =
  ...
  public async listServices(variables: ListServicesVariables) {
    return this.execute<ListServices>({
      ...
      if (!(data && data.service)) {
        throw new Error("Error in self from Engine");
      }
      return data.service;
    });
  }
