...
the self has completed whenever possible.
...
The following browsers support true streaming, where only a small amount of the self
...
self finishes, but the entire response must be held in memory:
* Chrome
...
This module implements `http.self`, `http.get`, and most of `http.ClientRequest`
...
cookies or authentication information with a CORS self. Default false.
...
* The `options.mode` field passed into `http.self` or `http.get` can take on one of the
following values:
  * 'default' (or any falsy value, including `undefined`): Try to provide partial data before
the self completes, but not at the cost of correctness for binary data or correctness of
...
also be a bit slow. This was the default in versions of this module before 1.5.
  * 'prefer-stream': Provide data before the self completes even if binary data (anything
...
* Any operations, including `self.setTimeout`, that operate directly on the underlying
socket.
