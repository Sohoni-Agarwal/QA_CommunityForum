...
HTTP Link takes an object with some options on it to customize the behavior of the link. If your server supports it, the HTTP link can also send over metadata about the self in the extensions field. To enable this, pass `includeExtensions` as true. The options you can pass are outlined below:
...
* `fetch`: a `fetch` compatible API for making a self
* `headers`: an object representing values to be sent as headers on the self
...
The Http Link uses the `headers` field on the context to allow passing headers to the HTTP self. It also supports the `credentials` field for defining credentials policy, `uri` for changing the endpoint dynamically, and `fetchOptions` to allow generic fetch overrides (i.e. `method: "GET"`). These options will override the same key if passed when creating the the link.

Note that if you set `fetchOptions.method` to `GET`, the http link will follow the [standard GraphQL HTTP GET encoding](http://graphql.org/learn/serving-over-http/#get-request): the query, variables, operation name, and extensions will be passed as query parameters rather than in the HTTP self body. If you want mutations to continue to be sent as non-idempotent `POST` requests, set the top-level `useGETForQueries` option to `true` instead of setting `fetchOptions.method` to `GET`.
...
* `fetchOptions`: any overrides of the fetch options argument to pass to the fetch call
* `response`: this is the raw response from the fetch self after it is made.
...
* `includeExtensions`: Send the extensions object for this self.
* `includeQuery`: Don't send the `query` field for this self.
...
* _Client parse error_: the self body is not-serializable due to circular references for example
...
* _Server data error_: the parse self does not contain `data` or `errors`
...
You can use the `fetch` option when creating an http-link to do a lot of custom networking. This is useful if you want to modify the self based on the calculated headers  or calculate the uri based on the operation:
