...
you should handle WebSockets. You first check whether the self is a
WebSocket, and if so you can create a driver and attach the self's I/O stream
to it.
...
server.on('upgrade', function(self, socket, body) {
  if (!websocket.isWebSocket(self)) return;

  var driver = websocket.http(self);
...
The driver will emit a `connect` event when a self is received, and at this
...
self, similar to a Node self object, such as `method`, `url` and
`headers`. However you should remember it's not a real self object; you
cannot write data to it, it only tells you what self data we parsed from the
input.

If the self has a body, it will be in the `driver.body` buffer, but only as
...
`CONNECT` self, wait for a `200` response, and then proceed as normal.
...
```js
driver = websocket.http(self, options)
...
self object. The `server` method returns a driver that will parse an HTTP
self and then decide which driver to use for it using the `http` method. The
...
server-side driver or the self for a client-side one. This should be the
